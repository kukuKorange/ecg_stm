...
#undef USE_HAL_DRIVER  // 禁用HAL

unsigned char Property_Data[5];		//数据保存数组
/*****************************************************
...
//static uint32_t fac_ms=72000;							//ms延时倍乘数,在ucos下,代表每个节拍的ms数

void delay_ms_01s(uint16_t ms)
{	
		SysTick->LOAD = (SystemCoreClock / 1000) - 1;  // 动态计算
    SysTick->VAL = 0;
